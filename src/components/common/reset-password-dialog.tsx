// src/components/common/reset-password-dialog.tsx
"use client";
...
interface ResetPasswordDialogProps {
    ...
    userEmail: string;
    userType: UserType;
    /** Solo requerido para customers que tienen user_id diferente al id */
...
/**
 * Componente genérico para resetear contraseña de specialists o customers
 */
export function ResetPasswordDialog({
    ...
    const userLabel = userType === "specialist" ? "especialista" : "cliente";

    const handleReset = async () => {
        // Verificar que tenga cuenta vinculada (para customers)
        if (userType === "customer" && !hasLinkedAccount) {
            toast.error("Este cliente no tiene cuenta vinculada. Use 'Crear cuenta' primero.");
            return;
        }
        ...
        setIsLoading(true);
        try {
            // Llamar a la API route correspondiente según userType
            const apiPath = userType === "specialist"
                ? `/api/specialists/${userId}/reset-password`
                : `/api/customers/${userId}/reset-password`;
    ...
    const handleClose = () => {
        ...
        if (newCredentials && onSuccess) {
            onSuccess();
        }
    };

    // Si es customer sin cuenta, mostrar mensaje
    if (userType === "customer" && !hasLinkedAccount) {
        return null; // El botón no se renderiza, usar LinkAccountDialog en su lugar
    }
                        ...
                        <div className="space-y-4 py-4">
                            ...
                            <Alert>
                                <AlertDescription className="text-sm">
                                    El {userLabel} deberá usar la nueva contraseña la próxima vez
                                    que inicie sesión.
                                </AlertDescription>
                            </Alert>
                        </div>
