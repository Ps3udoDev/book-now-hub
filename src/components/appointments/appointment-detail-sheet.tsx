...
export function AppointmentDetailSheet({
    ...
    const endDate = appointment.ends_at
        ? appointment.ends_at
        : new Date(new Date(appointment.scheduled_at || "").getTime() + (appointment.duration_minutes || 15) * 60000).toISOString();

    const timeRange = appointmentsService.formatTimeRange(
        appointment.scheduled_at || "",
        endDate
    );
    const dateStr = appointmentsService.formatDate(appointment.scheduled_at || "");
    ...
    function downloadClientICS() {
        ...
        const endDate = appointment.ends_at
            ...
            : new Date(new Date(appointment.scheduled_at).getTime() + (appointment.duration_minutes || 15) * 60000);
                ...
                <div className="mt-6 space-y-6 px-1">
                    ...
                    <div className="space-y-4">
                        ...
                        {appointment.estimated_price != null && (
                            <DetailRow
                                icon={<span className="text-muted-foreground text-sm font-mono">$</span>}
                                label="Precio"
                    ...
                    {(appointment.customer_notes || appointment.internal_notes) && (
                        <>
                            ...
                            <div>
                                ...
                                <p className="text-sm">{appointment.customer_notes || appointment.internal_notes}</p>
                            </div>
                        </>
                    )}
...
function getStatusActions(status: string) {
    switch (status) {
        case "pending":
            return [
                { key: "confirm", label: "Confirmar", icon: CheckCircle, variant: "default" },
                { key: "cancel", label: "Cancelar", icon: XCircle, variant: "destructive" },
            ];
        case "confirmed":
            return [
                { key: "start", label: "Iniciar", icon: PlayCircle, variant: "default" },
                { key: "no_show", label: "No asistió", icon: AlertTriangle, variant: "outline" },
                { key: "cancel", label: "Cancelar", icon: XCircle, variant: "destructive" },
            ];
        case "in_progress":
            return [
                { key: "complete", label: "Completar", icon: CheckCircle, variant: "default" },
            ];
        default:
            return [];
    }
